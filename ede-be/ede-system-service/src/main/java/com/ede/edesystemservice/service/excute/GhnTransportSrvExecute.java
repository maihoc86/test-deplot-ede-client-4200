...
public class GhnTransportSrvExecute implements TransportService {
	...
	public List<Ward> getWards(Optional<Integer> districtId) {
		if (!districtId.isPresent()) {
			throw new UnsupportedOperationException("Giao hàng nhanh Không hổ trợ truy xuất thông tin các phường/xã, mà không có thông tin quận/huyện kèm theo");
		}
	...
	public List<AvailableService> getAvailableServices(int fromDistrictId, int toDistrictId) {
		String urlGetAvailableService = String.format("%s?from_district=%d&to_district=%d&shop_id=%s",URL_GET_AVAILABLE_SERVICES, fromDistrictId, toDistrictId, SHOP_ID);
	...
	public TransportFee getTransportFee(int fromDistrictId, int toDistrictId, int serviceId, float weight) {
		String urlGetFee = String.format("%s?from_district_id=%d&to_district_id=%d&weight=%.0f&service_id=%d",URL_GET_TRANSPORT_FEE, fromDistrictId, toDistrictId, weight, serviceId);
	...
	@Override
	public TransportTime getTransportTime(int fromDistrictId, String fromWardId, int toDistrictId, String toWardId, int serviceId) {
		String urlGetTime = String.format("%s?"
				+ "from_district_id=%d&"
				+ "from_ward_code=%s&"
				+ "to_district_id=%d&"
				+ "to_ward_code=%s&"
